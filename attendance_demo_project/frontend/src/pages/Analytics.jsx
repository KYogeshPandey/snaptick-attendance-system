      ...
      <div className="container-fluid" style={{ maxWidth: '1400px' }}>
        ...
        <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
          ...
          <div className="d-flex gap-2">
            <button onClick={exportCSV} className="btn btn-success shadow-sm hover-lift">
              <i className="bi bi-file-earmark-spreadsheet me-2"></i>📊 Export CSV
            </button>
            <button onClick={exportPDF} className="btn btn-danger shadow-sm hover-lift">
        ...
        <div className="row g-4 mb-4">
          {[
            ...
            <div key={i} className="col-md-6 col-lg-3">
              <div className={`card border-0 shadow-sm h-100 bg-${card.bg} bg-opacity-10 border-start border-${card.bg} border-4 hover-lift`}>
                <div className="card-body">
                  <div className="d-flex justify-content-between align-items-start">
                    ...
                    <div className={`bg-${card.bg} rounded-circle d-flex align-items-center justify-content-center icon-bounce`} 
        ...
        {heatmapData && (
          <div className="card border-0 shadow-sm mb-4 hover-lift">
            <div className="card-body">
              ...
              <div className="overflow-auto bg-white rounded p-3 mb-3 border">
                <div style={{ minWidth: 600 }}>
                  <div className="d-flex mb-2" style={{ marginLeft: 60 }}>
                    {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((day, i) => (
                      <div key={i} className="text-muted fw-semibold small text-center" style={{ width: 28, marginRight: 4 }}>{day}</div>
                    ))}
                  </div>
                  {getWeekData(heatmapData.heatmap).map((week, weekIdx) => (
                    <div key={weekIdx} className="d-flex align-items-center mb-2">
                      <small className="text-muted fw-semibold text-end" style={{ width: 50, marginRight: 10 }}>W{weekIdx + 1}</small>
                      {week.map((day, dayIdx) => (
                        ...
                        className={`rounded heatmap-cell ${day.total_marked > 0 ? 'heatmap-clickable' : ''} ${selectedDay?.date === day.date ? 'heatmap-selected' : ''}`}
                        style={{
                          width: 28, height: 28, marginRight: 4,
                          background: getIntensityColor(day.intensity),
                          border: selectedDay?.date === day.date ? '3px solid #3b82f6' : '1px solid #d1d5db',
                          cursor: day.total_marked > 0 ? 'pointer' : 'default',
              ...
              {selectedDay && (
                <div className="alert alert-info d-flex align-items-center shadow-sm" role="alert">
                  <div>
                    <strong className="fs-5">{selectedDay.date} ({selectedDay.day_of_week})</strong>
                    <div className="mt-1">
                      Present: <strong className="text-success">{selectedDay.present}</strong> / {selectedDay.total_students} 
                      <span className="badge bg-info ms-2">{selectedDay.percentage}%</span>
                    </div>
                  </div>
                </div>
              )}

              <div className="d-flex align-items-center justify-content-center gap-3 mb-4 py-3 bg-light rounded">
                <small className="text-muted fw-bold">Less</small>
                {[0, 1, 2, 3, 4].map(level => (
                  <div key={level} className="rounded border shadow-sm" 
                       style={{ width: 28, height: 28, background: getIntensityColor(level) }} />
                ))}
              ...
              {heatmapData.stats && (
                <div className="row g-3 mt-2">
                  <div className="col-md-3">
                    <div className="card bg-light border-0 hover-scale">
                  ...
                  {heatmapData.stats.best_day && (
                    <div className="col-md-3">
                      <div className="card bg-success bg-opacity-10 border-success border-2 hover-scale">
                        <div className="card-body text-center">
                  ...
                  {heatmapData.stats.worst_day && (
                    <div className="col-md-3">
                      <div className="card bg-danger bg-opacity-10 border-danger border-2 hover-scale">
                        <div className="card-body text-center">
        ...
        {students.filter(s => s.attendance_rate < 50).length > 0 && (
          <div className="alert alert-danger border-0 shadow-sm mb-4 hover-lift">
            ...
            <div className="list-group">
              {students.filter(s => s.attendance_rate < 50).slice(0, 5).map(student => (
                <div key={student.student_id} className="list-group-item d-flex justify-content-between align-items-center hover-bg-light">
                  <div>
